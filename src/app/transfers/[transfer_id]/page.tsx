...
export default function TransferDetailPage({ params }: PageProps) {
    ...
    async function load() {
      ...
      try {
        const [transferRes, recordsetsRes] = await Promise.all([
          fetch(`/api/transfers/${transfer_id}`, { cache: "no-store" }),
          fetch(`/api/transfers/${transfer_id}/recordsets`, { cache: "no-store" }),
        ]);
        ...
        if (!transferRes.ok) {
          ...
          const msg =
            typeof json.error === "string"
              ? json.error
              : (json.error?.message ?? `Could not load transfer ${transfer_id}.`);
          throw new Error(msg);
        }
        ...
        // Fetch destination-specific settings after we know the abbr
        const settingsPath = SETTINGS_ENDPOINT[loadedTransfer.destination_abbr];
        if (settingsPath) {
          const settingsRes = await fetch(`/api/transfers/${transfer_id}/${settingsPath}`, { cache: "no-store" });
          if (!isMounted) return;
          if (settingsRes.ok) {
        ...
        if (!isMounted) return;
        setError(e instanceof Error ? e.message : `Could not load transfer ${transfer_id}.`);
        setTransfer(null);
      } finally {
  ...
  async function generateManifest(r: RecordsetRelease) {
    ...
    try {
      const res = await fetch(
        `/api/transfers/${transferId}/recordsets/${r.recordset_release_id}/manifest/generate`,
        { method: "POST" },
      );
      if (!res.ok) {
  ...
  async function generateIdcManifest(type: "dataset" | "recordset" | "clinical") {
    ...
    try {
      const res = await fetch(`/api/transfers/${transferId}/idc/${type}-manifest/generate`, { method: "POST" });
      if (!res.ok) {
        const json = (await res.json()) as { error?: { message?: string } | string };
  ...
  async function saveSettings(e: { preventDefault: () => void }) {
    ...
    try {
      const res = await fetch(`/api/transfers/${transferId}/${settingsPath}`, {
        method: "PUT",
        headers: { "content-type": "application/json" },
