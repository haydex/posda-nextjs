...
export default function ReleaseTransferCreatePage({ params }: PageProps) {
  ...
  // Load release info and destinations on mount
  useEffect(() => {
    let isMounted = true;
    ...
    async function loadInit() {
      ...
      try {
        const [releaseRes, destinationsRes] = await Promise.all([
          ...
          fetch(`/api/datasets/releases/${release_id}/destinations`, { cache: "no-store" }),
        ]);
  ...
  // Re-fetch recordset releases whenever the selected destination changes
  useEffect(() => {
    ...
    async function loadRecordsets() {
      ...
      try {
        const res = await fetch(
          `/api/datasets/releases/${releaseId}/recordsets?destination_id=${selectedDestinationId}`,
          { cache: "no-store" },
        );
