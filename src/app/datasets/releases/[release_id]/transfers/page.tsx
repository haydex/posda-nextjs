...
export default function ReleaseTransfersPage({ params }: PageProps) {
  ...
  async function loadTransfers(id: string) {
    const res = await fetch(`/api/datasets/releases/${id}/transfers`, { cache: "no-store" });
    ...
    async function load() {
      ...
      try {
        const [releaseRes, transfersRes] = await Promise.all([
          fetch(`/api/datasets/releases/${release_id}`, { cache: "no-store" }),
          fetch(`/api/datasets/releases/${release_id}/transfers`, { cache: "no-store" }),
        ]);
        ...
        if (loadedRelease?.dataset_id) {
          const datasetRes = await fetch(`/api/datasets/${loadedRelease.dataset_id}`, { cache: "no-store" });
  ...
  async function syncTransfers() {
    ...
    try {
      const destRes = await fetch(`/api/datasets/releases/${releaseId}/destinations`, { cache: "no-store" });
      ...
      const results = await Promise.allSettled(
        destinations.map(async (dest) => {
          const rsRes = await fetch(
            `/api/datasets/releases/${releaseId}/recordsets?destination_id=${dest.destination_id}`,
            { cache: "no-store" },
          );
          ...
          if (!existing) {
            const res = await fetch(`/api/datasets/releases/${releaseId}/transfers`, {
              method: "POST",
            ...
            const currentRes = await fetch(
              `/api/transfers/${existing.dataset_release_transfer_id}/recordsets`,
              { cache: "no-store" },
            );
            ...
            await Promise.all([
              toAdd.length > 0
                ? fetch(`/api/transfers/${existing.dataset_release_transfer_id}/recordsets/add`, {
                    method: "POST",
                    headers: { "content-type": "application/json" },
                ...
                : Promise.resolve(),
              toRemove.length > 0
                ? fetch(`/api/transfers/${existing.dataset_release_transfer_id}/recordsets/remove`, {
                    method: "POST",
                    headers: { "content-type": "application/json" },
      ...
      <SectionCard className="mt-1">
        ...
        {!isLoading && !error && (
          <DynamicTable
            ...
            onRowClick={(row) =>
              router.push(`/transfers/${row.dataset_release_transfer_id}`)
            }
